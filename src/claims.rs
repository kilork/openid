...
pub trait Claims {
    ...
    /// the nonce parameter sent in the Authentication Request. If present in
    /// the Authentication Request, Authorization Servers MUST include a nonce
    ...
    /// string. If the ID Token is issued from the Authorization Endpoint
    /// with a code, which is the case for the response_type values code
    /// id_token and code id_token token, this is REQUIRED; otherwise, its
    ...
    /// meet the requirements of ISO/IEC 29115 [ISO29115] level 1.
    ...
    /// nist_auth_level 0.) An absolute URI or an RFC 6711 [RFC6711] registered
    /// name SHOULD be used as the acr value; registered names MUST NOT be used
    /// with a different meaning than that which is registered. Parties using
    /// this claim will need to agree upon the meanings of the values used,
    ...
    /// instance, values might indicate that both password and OTP
    ...
    /// The userinfo method returns a reference to the Userinfo struct, which
    /// contains information about the user who issued the ID Token.
    fn userinfo(&self) -> &Userinfo;

    /// Decodes at_hash. Returns None if it doesn't exist or something goes
    /// wrong.
    ///
    /// See [spec 3.1.3.6](https://openid.net/specs/openid-connect-core-1_0.html#CodeIDToken)
    ///
    /// The returned Vec is the first 128 bits of the access token hash using
    /// alg's hash alg
    fn at_hash_to_vec(&self) -> Option<Vec<u8>> {
        URL_SAFE_ANYPAD.decode(self.at_hash()?).ok()
    }
    /// Decodes c_hash. Returns None if it doesn't exist or something goes
    /// wrong.
    ///
    /// See [spec 3.3.2.11](https://openid.net/specs/openid-connect-core-1_0.html#HybridIDToken)
    ///
    /// The returned Vec is the first 128 bits of the code hash using alg's hash
    /// alg
